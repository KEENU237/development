...
class BasketOrder:
    ...
    def max_profit(self) -> float:
        ...
        return None  # Calculate karna hoga strategy-wise
    ...
    def summary(self) -> str:
        ...
        for leg in self.legs:
            ...
            lines.append(
                ...
                f"x{leg.qty}  LTP: ₹{leg.ltp:.1f}  "
                f"Premium: ₹{abs(leg.premium):,.0f} ({sign})"
            )
        net_sign = "DEBIT" if self.net_premium > 0 else "CREDIT"
        lines.append(f"{'─'*50}")
        lines.append(f"  Net {net_sign}: ₹{abs(self.net_premium):,.0f}")
        lines.append(f"{'─'*50}\n")
        return "\n".join(lines)
...
class BasketOrderBuilder:
    ...
    def build_bull_call_spread(self, symbol: str, expiry: str,
        ...
        """
        ATM CE buy + OTM CE sell
        Net debit strategy — limited risk, limited reward
        """
        ...
        if abs(order.net_premium) > RISK["max_capital_per_trade"]:
            logger.warning(f"Risk limit exceed: ₹{abs(order.net_premium):,} > ₹{RISK['max_capital_per_trade']:,}")
            return None
    ...
    def build_bear_put_spread(self, symbol: str, expiry: str,
                               lot_size: int) -> Optional[BasketOrder]:
        """
        ATM PE buy + OTM PE sell
        Net debit — bearish with defined loss
        """
    ...
    def build_iron_condor(self, symbol: str, expiry: str,
                           lot_size: int) -> Optional[BasketOrder]:
        """
        4-leg net credit strategy:
        Sell OTM CE + Buy far OTM CE
        Sell OTM PE + Buy far OTM PE
        """
    ...
    def _get_spot(self, symbol: str) -> Optional[float]:
        exchange_sym = f"NSE:{symbol} 50" if symbol == "NIFTY" else f"NSE:{symbol}"
